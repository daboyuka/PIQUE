...
template<typename RegionEncoderT, typename BinningSpecT, typename BinMergeSetOperationsT>
static void do_test(const TestCase &test, typename RegionEncoderT::RegionEncoderConfig conf, boost::shared_ptr< BinningSpecT > binning_spec, IndexEncoding::Type enc_type, typename BinMergeSetOperationsT::SetOperationsConfig binmerge_setops_conf, std::map< std::string, boost::shared_ptr< QueryEngine > > qes, std::string indexfile, std::string datametafile) {
...
int main(int argc, char **argv) {
	...
	boost::shared_ptr< SigbitsBinningSpecification< int > > sigbits_binning = boost::make_shared< SigbitsBinningSpecification< int > >(sizeof(int)*8);
	boost::shared_ptr< ExplicitBinsBinningSpecification< int > > eb_binning = boost::make_shared< ExplicitBinsBinningSpecification< int > >(std::vector<int>{ 1, 2, 3, 4, 5, 8, 10, 20, 40, 60, 80, 100, 120, 140, 160, 180 });
	...
	for (auto test_it = TEST_CASES.cbegin(); test_it != TEST_CASES.cend(); test_it++) {
		...
		for (auto it = encs.begin(); it != encs.end(); ++it) {
			...
			do_test< CBLQRegionEncoder<2>, SigbitsBinningSpecification<int>, CBLQSetOperationsNAry3Fast<2> >(test, CBLQRegionEncoderConfig(false), sigbits_binning, enc_type, CBLQSetOperationsConfig(true), qes, indexfile, datametafile);
			do_test< CBLQRegionEncoder<2>, ExplicitBinsBinningSpecification<int>, CBLQSetOperationsNAry3Fast<2> >(test, CBLQRegionEncoderConfig(false), eb_binning, enc_type, CBLQSetOperationsConfig(true), qes, indexfile, datametafile);
		}
	}
}
