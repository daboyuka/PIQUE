...
/*
 * bitmap-query-engine.hpp
 *
 *  Created on: Aug 7, 2014
 *      Author: David A. Boyuka II
 */
#ifndef BITMAP_QUERY_ENGINE_HPP_
#define BITMAP_QUERY_ENGINE_HPP_
...
class BitmapQueryEngine : public SimpleQueryEngine {
public:
	using QueryEngine::QueryStats;
	using QueryEngine::QuerySummaryStats;

	BitmapQueryEngine(
			boost::shared_ptr< FixedOutputRegionEncodingConverter< BitmapRegionEncoding > > bitmap_converter,
			boost::shared_ptr< SetOperations< BitmapRegionEncoding > > bitmap_setops,
			QueryEngineOptions options = QueryEngineOptions()) :
		SimpleQueryEngine(options), bitmap_converter(bitmap_converter), bitmap_setops(bitmap_setops) {}
	virtual ~BitmapQueryEngine() {}
...
#endif /* BITMAP_QUERY_ENGINE_HPP_ */
